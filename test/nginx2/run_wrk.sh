...
svc(){
  ...
  echo "sleeping 10s"
  sleep 10
  echo "nohup ans/build/ans --file-prefix=wrks $PCI $MEM $CPU > ans.wrk.log 2>&1 &"
  nohup ans/build/ans --file-prefix=wrks $PCI $MEM $CPU > ans.wrk.log 2>&1 &
  echo "starting 10s"
  sleep 10
  #cli/build/anscli --file-prefix=wrks "ip addr add $IP/24 dev veth0"
  #sleep 10
}
...
mon1(){
  ...
  T=$2
  C=$3
  LOG_CPU=logs/mpstat.client_$T_$C.log
  mpstat -P $CPU $INT > $LOG_CPU 2>&1 &
  RPSa=`tail -f $CLT | grep -m 1 'Req/Sec'|awk '{print $2}'`
  RPSx=`grep 'Req/Sec' $CLT|awk '{print $4}'`
  LAT50=`grep ' 50%' $CLT |awk '{print $2}'`
  LAT75=`grep ' 75%' $CLT |awk '{print $2}'`
  ...
  LATx=`grep Latency $CLT|grep -v Dist |awk '{print $4}'`
  LATx=${LATx/.00/}
  CPUs=`tail -n1 $LOG_CPU|awk '{printf("%d+%d",$3,$5)}'`
  CPUc=`tail -n2 $LOG_CPU|head -1|awk '{printf("%d+%d",$3,$5)}'`
  echo "$T      $C      $RPSa   $RPSx   $LAT50  $LAT75  $LAT90  $LAT99  $LATx   $CPUc   $CPUs"
}
wrk(){
  IP=$1
